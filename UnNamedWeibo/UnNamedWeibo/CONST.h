//
//  CONST.h
//  UnNamedWeibo
//
//  Created by Kitten Yang on 2/16/15.
//  Copyright (c) 2015 Kitten Yang. All rights reserved.
//

#ifndef UnNamedWeibo_CONST_h
#define UnNamedWeibo_CONST_h


#endif

#define kWeiboAppKey     @"1532246631"
#define kWeiboAppSecret  @"93eedeccc67d099354370679ef8c906e"
#define kWeiboRedirectURI @"https://api.weibo.com/oauth2/default.html"
#define kWeiboURLSchemes @"wb1532246631"
#define kWeiboAuthSuccessNotification @"kWeiboAuthSuccessNotification" //微博登录成功的通知


//WEIBO API
#define WB_home   @"https://api.weibo.com/2/statuses/home_timeline.json"
